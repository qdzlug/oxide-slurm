...
resource "local_file" "ansible_inventory" {
  ...
  content = templatefile("${path.root}/templates/hosts.ini.tpl", {
    ...
    compute_ips = [
      for key, instance in data.oxide_instance_external_ips.slurm :
      instance.external_ips[0].ip if tonumber(key) > 0
    ]
  })
}
...
/*# Run Ansible Playbook after Terraform Applies
resource "null_resource" "run_ansible" {
  depends_on = [local_file.ansible_inventory]

  provisioner "local-exec" {
    command = <<EOT
      ansible-playbook -i ../ansible/inventory.ini deploy_slurm.yml
    EOT
  }
}*/
